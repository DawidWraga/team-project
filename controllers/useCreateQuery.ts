...
>(
  ...
  const { queryFn, fetcherConfig: inputFetcherConfig, passQuery, ..._options } = options;

  const fetcherConfig = { ...inputFetcherConfig };
  ...
  if (queryFn && fetcherConfig) defaultOptions.queryFn = () => queryFn(fetcherConfig);
